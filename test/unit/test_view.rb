...
class TestView < Test::Unit::TestCase
  ...
  def test_view_create
    design_doc = View.create( 'something/all', @fake_view )
  ...
  def test_view_update
    ...
    raw_doc = Document.get( '_design/something' )
    assert_equal new_map, raw_doc.views['all']['map']
  end

  def test_map_view_run_with_include_docs
    View.create( 'something/all', @fake_view )
    results = View.run( 'something/all', { :include_docs => true } )
    assert_equal 0, results.total_rows

    # create a doc which will be returned
    doc = Document.new( { :time_for => 'some thrilling heroics' } )
    doc.save
  ...
  def test_map_view_run
    ...
    results = View.run( 'something/all' )
    assert_equal 1, results.total_rows
  ...
  def test_reduce_view_run
    ...
    View.create( 'something/total', view )

    # run it
    results = View.run( 'something/total', { :reduce => true } )
  ...
  def test_group_view_run
    ...
    view = { :map => "function( doc ) { emit( doc.letter, doc.count ); }",
             :reduce => "function( keys, values ) { return sum( values ); }" }
    View.create( 'something/grouped', view )

    # run it
    results = View.run( 'something/grouped', { :group => true } )
